...
# Part I: Creating Data Cubes from Satellite Image Collections
...
Plotting an identical data cube twice, different with visualization arguments `zlim`, `col`, and others will not need to reprocess the data cube again. `plot()` internally writes netCDF files to a temporary directory and remembers that a specific cube is already available.
...
Sometimes we want to process data cubes further, e.g. with external software. We can export data cubes either as single netCDF files, or as a collection of GeoTIFF files, where each time-slice of the cube will be stored as one (multiband) file.
...
gdalcubes is a pretty young tool, there are many ideas still to be implemented (e.g. Python interface, user-defined function support for further operations, interfacing image processing library such as Orfeo Toolbox, using gdalcubes as a fully open source [OpenEO](https://openeo.org/) backend, ...).  
...
1. Use the downsampled Landsat dataset from the first exercises, the data cube view and mask below, and create a data cube for a spatial subarea. Y
